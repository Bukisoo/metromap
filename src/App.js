...
const App = () => {
  ...
  const [lineNumberWidth, setLineNumberWidth] = useState(50);
  ...
  const updateLineCount = () => {
    const lineCount = textareaRef.current ? textareaRef.current.value.split('\n').length : 1;
    setLineCount(lineCount);
    const newWidth = (lineCount.toString().length * 10) + 10;
    setLineNumberWidth(newWidth);
  };
  ...
  return (
    <div className="app-container">
      ...
      <div className={`editor-container ${isEditorVisible ? 'visible' : ''}`} ref={editorRef} onClick={(e) => e.stopPropagation()}>
        {selectedNode && (
          <div className="editor-content">
            ...
            <div className="textarea-container">
              <div className="line-numbers" ref={lineNumberRef} style={{ width: `${lineNumberWidth}px` }}>
                {Array.from({ length: lineCount }, (_, i) => (
                  <div key={i}>{String(i + 1).padStart(4, ' ')}</div>
                ))}
              </div>
              <textarea
                ref={textareaRef}
